...
class product_product(models.Model):
    ...
    rent_per_month = fields.Float('Monthly Rental', default=0.0,help="Rental per month.")
    rent_per_week = fields.Float('Weekly Rental', default=0.0,help="Rental per week.")
    rent_per_day = fields.Float('Daily Rental', default=0.0,help="Rental per day.")
    rent_per_hour = fields.Float('Hourly Rental', default=0.0,help="Hourly Rent.")
    replacement_value = fields.Float('Replacement Value', readonly=True, help="Replacement Value")
    weekly_replacement_value = fields.Float('Weekly Replacement Value', readonly=True, help="Week Replacement Value")
    daily_replacement_value = fields.Float('Daily Replacement Value', readonly=True, help="Day Replacement Value")
    description_rental = fields.Text(string="Rental Description", required=False, )

    @api.model
    def get_view(self, view_id=None, view_type='form', **options):
        res = super(product_product, self).get_view(view_id, view_type, **options)
        doc = etree.XML(res['arch'])
        if view_type == 'form':
            for span in doc.xpath("//span[@class='d-inline-flex']"):
                rent_ok_field = span.xpath(".//field[@name='rent_ok']")
                if rent_ok_field:
                    span.set('invisible', '1')
...
class ResPartner(models.Model):
    ...
    def _get_rental_count(self):
        for res in self:
            rental_ids = self.env['rental.order'].sudo().search([('partner_id', '=', res.id)])
