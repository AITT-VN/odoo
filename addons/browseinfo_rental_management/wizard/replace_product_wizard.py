...
class RentalRenew(models.TransientModel):
    ...
    date = fields.Datetime(string="New Extended Date", required=True)
    ...
    @api.constrains('rental_initial')
    def rental_initial_constrains(self):
        rental = self.env['rental.order'].browse(self._context.get('active_id'))
        if rental.rental_initial > self.rental_initial:
            raise UserError(_('Sorry !!! You Cannot Select Past Date!!!'))
...
class RentalProductReplace(models.Model):
    ...
    def default_get(self, fields):
        ...
        if 'existing_product_ids' in fields:
    ...
    def replace_product(self):
        ...
        for rl in self.existing_product_ids:
            ...
            rh_ids = self.env['rental.history'].sudo().search(
        ...
        if replace_p_id in exist_p_id :
            raise UserError(_('You have to select Same Product!'))

        
        check_replace = self.existing_product_ids.mapped('replace_item')
        if all(item is False for item in check_replace):
            raise ValidationError(_("Please Select Replace Product In Existing Products"))

        for line in self.existing_product_ids:
            if line.replace_item == True and line.ro_line_id:
                is_replace.append(line.id)
                if line.ro_line_id.product_id.id in replace_p_id:
                    if line.ro_line_id:
                        find_stock_move = self.env['stock.move'].sudo().search([('product_id','=',line.ro_line_id.product_id.id),('origin','=',line.ro_line_id.rental_id.display_name)])
                        line.ro_line_id.unlink()
                        if find_stock_move:
                            find_stock_move.picking_id.action_cancel()
        ...
        if replace_lines:
            for line in self.env['rental.order.line'].browse(replace_lines):
                # i._action_launch_procurement_rule_custom()
                ...
                if line.product_id.type in ['consu','product'] :
                    ...
                    for move in  picking.move_ids:
                        ...
                        if picking.move_ids.rental_line_id:
